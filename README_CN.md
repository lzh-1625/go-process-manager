...
日志检索框中的字符串会按 **类 shell 分词** 解析：空格分隔多个条件，可用双引号把含空格的片段包成一项（实现上使用 [`shlex`](https://pkg.go.dev/github.com/google/shlex)）。多个条件之间为 **AND**：所有正向条件都要满足，否定条件用于排除。

| 前缀 | 含义 |
|------|------|
| 无前缀 | **全文匹配（Match）**，与加 `^` 等价，默认按全文匹配处理。 |
| `^` | **全文匹配（Match）**，在日志字段上做全文类匹配。 |
| `~` | **通配 / 子串（WildCard）**，内部按 `*内容*` 形式在 keyword 字段上做匹配（ES / Bleve）。 |
| `!` | **否定全文（NotMatch）**，`!` 后面整段作为要排除的全文匹配。 |
| `!^` 或 `^!` | **否定全文（NotMatch）**，与 `!` 同类，显式写法。 |
| `!~` 或 `~!` | **否定通配（NotWildCard）**，排除日志匹配 `*内容*` 通配模式的行。 |

示例（空格分隔多个独立条件）：

- `error timeout` — 需同时满足对 *error* 与 *timeout* 的匹配语义。
- `"out of memory"` — 带空格的一段作为一个检索词。
- `~GET /api` — 通配/子串语义下的模式（具体规则以后端实现为准）。
- `error !debug` — 需匹配 *error*，且不能匹配 *debug*。

实现位于 `internal/app/repository/search`；**SQLite**、**Elasticsearch**、**Bleve** 三种后端在分词/通配等细节上会有差异。
