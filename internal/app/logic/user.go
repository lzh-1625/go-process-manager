...
func (u *UserLogic) EditUser(user model.User, currentAccount string, currentRole types.Role) error {
	if user.Account == DefaultRootAccount {
		return errors.New("can not edit root user")
	}
	if currentRole != types.RoleRoot && user.Account != "" {
