<template>
  ...
  <div v-else>
    <h6 class="text-h6 font-weight-bold pa-5 d-flex align-center">
      ...
      <v-btn color="primary" variant="tonal" size="small" @click="addTaskBefore">
    ...
    <v-table class="pa-3">
      ...
      <tbody>
        <tr v-for="item in paginatedTasks" :key="item.id">
          ...
          <td>
            <svg v-if="item.running" width="20" height="20" viewBox="0 0 48 48" fill="#000000">
          ...
          <td>
            <v-switch color="primary" @change="edit(item)" v-model="item.enable" density="compact" inset
              hide-details></v-switch>
          </td>
          <td>
            <v-switch color="primary" @change="edit(item)" v-model="item.apiEnable" density="compact" inset
              hide-details></v-switch>
          </td>
          <td>
    ...
    <v-card class="rounded-xl">
      ...
      <v-card-text class="pt-6">
        <v-container fluid>
          <v-row dense>
            <v-col cols="12" sm="12">
              <v-text-field :label="$t('taskPage.taskName')" variant="outlined" density="comfortable"
                v-model="taskForm.name" />
            </v-col>

            <v-col cols="12" sm="6">
              <v-autocomplete :label="$t('taskPage.judgeTarget')" item-title="name" item-value="value"
                variant="outlined" density="comfortable" :items="processSelect" v-model="taskForm.processId" />
            </v-col>

            <v-col cols="12" sm="6">
              <v-autocomplete :disabled="taskForm.processId == null" :label="$t('taskPage.judgeCondition')"
                item-title="name" item-value="value" variant="outlined" density="comfortable" :items="conditionSelect"
                v-model="taskForm.condition" />
            </v-col>

            <v-col cols="12" sm="6">
              <v-autocomplete :label="$t('taskPage.operationTarget')" item-title="name" item-value="value"
                variant="outlined" density="comfortable" :items="processSelect" v-model="taskForm.operationTarget" />
            </v-col>

            <v-col cols="12" sm="6">
              <v-autocomplete :disabled="taskForm.operationTarget == null" :label="$t('taskPage.executeOperation')"
                item-title="name" item-value="value" variant="outlined" density="comfortable" :items="operationSelect"
                v-model="taskForm.operation" />
            </v-col>

            <v-col cols="12" sm="6">
              <v-autocomplete :label="$t('taskPage.triggerTarget')" item-title="name" item-value="value"
                variant="outlined" density="comfortable" :items="processSelect" v-model="taskForm.triggerTarget" />
            </v-col>

            <v-col cols="12" sm="6">
              <v-autocomplete :disabled="taskForm.triggerTarget == null" :label="$t('taskPage.triggerEvent')"
                item-title="name" item-value="value" variant="outlined" density="comfortable" :items="eventSelect"
                v-model="taskForm.triggerEvent" />
            </v-col>

            <v-col cols="12" sm="6">
              <v-autocomplete :label="$t('taskPage.nextTask')" item-title="name" item-value="value" variant="outlined"
                density="comfortable" :items="taskSelect" v-model="taskForm.nextId" />
            </v-col>

            <v-col cols="12" sm="6">
              <v-text-field :label="$t('taskPage.cronJob')" variant="outlined" density="comfortable"
                placeholder="* * * * *" v-model="taskForm.cron" />
            </v-col>

            <v-col cols="12" v-if="!isAdd">
              <v-text-field label="API" variant="outlined" density="comfortable" readonly v-model="apiUrl"
                append-inner-icon="mdi-content-copy" @click:append-inner="copyToClipboard">
                <template v-slot:append>
                  <v-btn v-if="taskForm?.key == null" @click="changeApi" color="primary" variant="tonal" size="small"
                    icon="mdi-plus">
                  </v-btn>
                  <v-btn v-else @click="changeApi" color="primary" variant="tonal" size="small" icon="mdi-refresh">
                  </v-btn>
                </template>
              </v-text-field>
            </v-col>
